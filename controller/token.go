...
func GetTokenUsage(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"data": gin.H{
			...
			"total_available":      token.RemainQuota,
			"total_granted_usd":    quotaToUSD(token.RemainQuota + token.UsedQuota),
			"total_used_usd":       quotaToUSD(token.UsedQuota),
			"total_available_usd":  quotaToUSD(token.RemainQuota),
			"amount_unit":          "USD",
