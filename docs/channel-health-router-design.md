...
### 3.1 原子评级（逐请求）= 内置色标 + 组合判定
两维度阈值复用 `web/default/src/features/usage-logs/lib/format.ts` 的官方阈值（与用户日志里看到的颜色一致）：

| 维度 | 🟢 green(0) | 🟡 yellow(1) | 🔴 red(2) |
|---|---|---|---|
| 首字 `frt` | < 5s | 5–10s | ≥ 10s |
| 流速 `completion_tokens/生成秒`（输出≥100 token） | ≥ 30 | 15–30 | < 15 |
| 总时长（输出<100 token） | < 10s | 10–30s | ≥ 30s |

**组合判定（v2.1 修正，非取最差）**：`sum = 首字色 + 响应色`，`sum==0` 绿 / `sum∈{1,2}` 黄 / `sum>=3` 红。
即**单一维度差只算黄，需两维度互相佐证（至少一黄一红）才判红**。

> 为什么改：实测「首字红但响应绿」的请求 ch61 占 24.9%、ch80 占 25.4%、ch49 占 18.5%，
> 而「响应红但首字绿」普遍 <2% —— 噪声几乎全来自首字波动（网络抖动）。取最差会把这些
> 误判为红；组合判定把它们降级为黄，红色率普降 70-80%（ch68 15.8%→4.8%）。
> **例外**：无首字（流式完全没收到首字，frt=-1000 哨兵）是硬故障，不参与组合佐证，直接计红。

### 3.2 渠道健康 = 最近 20 次请求的红色计数
对每个渠道维护一个长度 20 的环形缓冲（计数窗口），统计其中 `lvl==red` 的数量 `red_cnt`（同时可记 `green_cnt` 供展示）。

### 3.3 状态判定（带连续确认与滞回）
> 阈值已按组合判定重新标定（红色率腰斩后下调）。健康渠道均红/20 ≤1，坏渠道 2.5-3.2，`BadRedCnt=3` 干净分开。

| 状态 | 触发条件 | 路由动作 |
|---|---|---|
| **OK** | `red_cnt ≤ 1` | 正常路由，最便宜优先 |
| **WARN** | `red_cnt == 2` | 早警（"黄色多了"）：分流**小比例探测流量**到备选渠道，观察是否更优 |
| **BAD** | `red_cnt ≥ 3` 且**连续 ≥3 个评估窗**保持 | 从正常选路候选**软排除**，进入冷却 |
| **COOLDOWN/PROBING** | 进入 BAD 后 | 排除出正常池，仅按概率放探测流量回该渠道 |

恢复：冷却到期转 PROBING；探测样本按单次结果累计连胜（绿/黄+1，红清零），连续 `RecoverOKStreak` 次 → 回 OK（**立即切回低成本**）；探测仍红 → 冷却 `×2` 退避至上限。
...
| ③ 组合判定 | 单维度差（多为首字网络波动）仅计黄，需两维度佐证才计红 | 「首字红响应绿」ch61 占 24.9%，组合后红率降 70-80% |
