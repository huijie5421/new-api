...
function countHeaders(headers: string): number {
  if (!headers || !headers.trim()) return 0
  try {
      ...
      <div className='flex items-start justify-between gap-3'>
        <div className='min-w-0 flex-1 space-y-1'>
          <div className='flex flex-wrap items-center gap-1.5'>
            ...
            {template.is_default && (
              <Badge className='bg-amber-500/15 gap-1 text-amber-600 dark:text-amber-400'>
...
export function TemplateManagerDialog({
  ...
  useEffect(() => {
    ...
    for (const tpl of visible) {
      if (monitorCounts[tpl.id] !== undefined) continue
      channelMonitorAPI
      ...
      <Dialog
        ...
        title={
          <span className='flex items-center gap-2'>
            <span className='bg-gradient-to-br from-blue-500 to-indigo-600 flex size-7 items-center justify-center rounded-lg shadow-sm'>
